...
Uses
  ...
  Vcl.Forms, Vcl.Dialogs, strUtils, GenericEntidade,Dateutils;
...
class procedure TGenericDAO.RegistrarLog(CodigoPessoa, Modulo, log, Operacao, Tabela : string);
begin
  {if Modulo <> '' then
     DSServerModuleBaseDados.execSql('INSERT INTO Log '+
                                   ' ("CodigoFuncionario" ,"Data", "CodigoModulo", "Tabela"'+
...
begin
           ...
           begin
              ...
              begin
                ...
                if YearOf( TDateTime( Prop.GetValue(TObject(Obj)).AsVariant) ) > 1899  then
                    ...
                    quotedstr(FormatDatetime('DD/MM/YYYY HH:MM:SS',TDateTime(Prop.GetValue(TObject(Obj)).AsVariant)))+ ','
                else
   ...
   RegistrarLog(CodigoPessoa, GetModulo(Obj), Log , 'Alteração', Tabela );
...
begin
   ...
   RegistrarLog(CodigoPessoa, GetModulo(Obj), Log , 'Exclusão', GetTableName(Obj) );
end;
