...
begin
  ...
  srcCobranca.dataset := DataSetCobranca;
   ...
   begin
      ...
      FillCombobox( tpCFOP, cboCFOP, 'Descricao like (''DEVOLUÇÃO DE COMPRA%'') OR '+
                                     'CFOP in (''5949'',''1949'')',
                                     'CFOP','( CFOP +''  '' +  Descricao) as Descricao ','CFOP');
...
begin
  ...
  if Erros <> '' then
  begin
     Memo1.lines.add('Erro na validação! Motivo: '+ Erros );
     showmessage('Erro na validação! Motivo: '+ Erros );
     abort;
  end;
...
begin
  ...
  case ACBrNFe1.Status of
    ...
    stNfeInutilizacao : frmStatus.lblStatus.Caption := 'Enviando pedido de Inutilização...';
    ...
    stNFeCCe :  frmStatus.lblStatus.Caption := 'Enviando Carta de Correção...';
...
begin
  ...
  begin
      ...
      begin
         ...
         begin
            showmessage('Informe a IE do destinatário');
            abort;
         end;
      end
      else
...
begin
  ...
  begin
    ...
    begin
       ...
       begin
         showmessage('CFOP não selecionado CFOP!');
         abort;
       end;
    ...
    begin
      ...
      begin
         ...
         Prod.nItem    := FDataSetProdutos.fieldByname('Ordem').asInteger; // Número sequencial, para cada item deve ser incrementado
         ...
         Prod.NCM      := FDataSetProdutos.fieldByname('CodificacaoNCM').asstring;//'94051010'; // Tabela NCM disponível em  http://www.receita.fazenda.gov.br/Aliquotas/DownloadArqTIPI.htm
         Prod.EXTIPI   := '';
...
begin
   ...
   begin
     showmessage('Marque o Indicador da IE do destinatário');
     abort;
   end;
   ...
   begin
     showmessage('Necessário informar dados da NotaReferenciada!');
     abort;
   end;}
