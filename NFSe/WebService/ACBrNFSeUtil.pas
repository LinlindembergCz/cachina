...
uses
  ...
  {$ELSE}
    StrUtils,
  {$ENDIF}
  Sysutils, ACBrNFSeConfiguracoes, pnfsConversao, pnfsNFSe, pcnAuxiliar, ACBrDFeUtil, ActiveX;
...
begin
 ...
 if ALote
  then begin
   ...
   if (URI = '') or (AProvedor in [proRecife, {proRJ,} proAbaco, proIssCuritiba, proFISSLex, proSalvador])
    then AID := '>'
...
begin
 ...
 if ALote
  then begin
   ...
   if (URI = '') or (AProvedor in [proRecife, proRJ, proAbaco, proIssDSF, proIssCuritiba, proFISSLex, proSalvador])
    then AID := '>'
    else if AProvedor = proNatal then AID := ' ' + Identificador + '="Ass_lote">'
    else AID := ' ' + Identificador + '="AssLote_' + URI + '">';

   // Incluido por Italo em 07/08/2013           
   if AProvedor in [proAbaco, proIssCuritiba, proFISSLex, ProSalvador]
    then URI := '';
   ...
   AXML := AXML + '<Signature xmlns="http://www.w3.org/2000/09/xmldsig#"' + AID +
                 '<SignedInfo>' +
                  ...
                  '<Reference URI="' + DFeUtil.SeSenao(URI = '', '">', '#' + URI + '">') +
                   '<Transforms>' +
                    '<Transform Algorithm="http://www.w3.org/2000/09/xmldsig#enveloped-signature" />' +
                      DFeUtil.SeSenao((AProvedor in [profintelISS, proGovBr, proISSNet, proNatal, proIssDSF{, proSalvador}]), '',
                    '<Transform Algorithm="http://www.w3.org/TR/2001/REC-xml-c14n-20010315" />') +
                   '</Transforms>' +
                   '<DigestMethod Algorithm="http://www.w3.org/2000/09/xmldsig#sha1" />' +
                   '<DigestValue></DigestValue>' +
  ...
  //      raise Exception.Create('Não encontrei inicio do URI: Id=');
     ...
     if (URI = '') or (AProvedor in [profintelISS, proRecife, proNatal, proRJ{, proSalvador}]) then
       AID := '>'
     else
       if AProvedor = proSalvador then
         AID := ' ' + Identificador + '="' + URI + '">'
       else
         AID := ' ' + Identificador + '="Ass_' + URI + '">';

     if AProvedor = proSalvador then
       AXML := StringReplace(AXML,'InfRps id=','InfRps Id=',[rfReplaceAll]);
     ...
     Assinatura := '<Signature xmlns="http://www.w3.org/2000/09/xmldsig#"' + AID +
                    '<SignedInfo>' +
                     ...
                     '<Reference URI="' + DFeUtil.SeSenao(URI = '', '">', '#' + URI + '">') +
                      '<Transforms>' +
                       '<Transform Algorithm="http://www.w3.org/2000/09/xmldsig#enveloped-signature" />' +
                       DFeUtil.SeSenao((AProvedor in [profintelISS, proGovBr, proISSNet, proNatal{, proSalvador}]), '',
                       '<Transform Algorithm="http://www.w3.org/TR/2001/REC-xml-c14n-20010315" />') +
                      '</Transforms>' +
                      '<DigestMethod Algorithm="http://www.w3.org/2000/09/xmldsig#sha1" />' +
                      '<DigestValue></DigestValue>' +
 ...
 begin
   raise Exception.Create('Não foi possível carregar o arquivo: ' + AXML);
 end;
 ...
 if ALote
  then begin
   ...
   else if (URI <> '') and not (AProvedor in [proRecife, proRJ, proAbaco, proSalvador, proIssCuritiba, proFISSLex, proNatal])
    then xmldsig.signature := xmldoc.selectSingleNode('.//ds:Signature[@' + Identificador + '="AssLote_' + URI + '"]')
    else if AProvedor = proNatal then
      xmldsig.signature := xmldoc.selectSingleNode('.//ds:Signature[@' + Identificador + '="Ass_' + URI + '"]')
    else begin
...
//                   XMLAssinado := StringReplace(XMLAssinado, '</KeyInfo>', '', []);
 ...
 if xmlHeaderAntes <> ''
  ...
  if (not ALote) and (Aprovedor = proSalvador) then
    XMLAssinado[64] := 'i';
